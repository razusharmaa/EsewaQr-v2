...
const Bank = useSelector((state) => state.bank);
  ...
  const handleFileChange = (event) => {
    ...
    reader.onload = (event) => {
      ...
      qr.decodeFromImage(event.target.result).then((res) => {
        console.log('QR decode result', res);
        if(res){
          setQrResult(res.data);
          
        } else {
          dispatch(setAlert({message: 'QR decode failed', type: 'warning'}));
        }
      }).catch((err) => {
        dispatch(setAlert({message: 'QR decode error: ' + err.message, type: 'failure'}));
        console.log('QR decode error', err);
        
      });
    };
    reader.onerror = (err) => {
      dispatch(setAlert({message: 'File read error:'+err, type: 'failure'}));
  ...
  return (
    <div className="px-3 my-1 flex flex-col w-full items-center justify-center">
      <p className='py-1 text-xl font-mono font-medium'>Upload your Qr code here</p>
      <Label
